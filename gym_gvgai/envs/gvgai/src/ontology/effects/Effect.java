...
public abstract class Effect {
    ...
    // indicates whether the interactions of this effect should be carried out
    ...
    // Indicates the number of repetitions of this effect. This affects how many
    // times this
    // effect is taken into account at each step. This is useful for chain effects
    // (i.e. pushing
    ...
    /**
     * Indicates if the effect wishes to take into account all sprites of the second
     * type at once.
     */
    ...
    /**
     * Takes a batch of sprites to collide against at once, and builds the collision
     * boundary with them.
     *
     * @param sprite1     Sprite that collides with the batch
     * @param sprite2list Sprites to collide against.
     * @param game        our game.
     * @return number of sprites in the list to collide with. List comes back sorted
     *         by proximity.
     */
    ...
    public int getScoreChange(int playerID) {
        ...
        try {
            ...
            Logger.getInstance().addMessage(
                    new Message(Message.WARNING, "scoreChange must be an integer number not " + scoreChange + "."));
            return 0;
        }
    }
    ...
    public int getCounter(int idx) {
        ...
        try {
            ...
            Logger.getInstance()
                    .addMessage(new Message(Message.WARNING, "counter must be an integer number not " + counter + "."));
            return 0;
        }
    }
    ...
    public int getCounterElse(int idx) {
        ...
        try {
            ...
            Logger.getInstance().addMessage(
                    new Message(Message.WARNING, "counterElse must be an integer number not " + counterElse + "."));
            return 0;
        }
    }
...
//            }
    ...
    private Rectangle adjustLeft(VGDLSprite sprite1, VGDLSprite sprite2) {
        return new Rectangle(sprite2.rect.x + sprite2.rect.width, sprite1.rect.y, sprite1.rect.width,
                sprite1.rect.height);
    }

    private Rectangle adjustUp(VGDLSprite sprite1, VGDLSprite sprite2) {
        return new Rectangle(sprite1.rect.x, sprite2.rect.y + sprite2.rect.height, sprite1.rect.width,
                sprite1.rect.height);
    }
