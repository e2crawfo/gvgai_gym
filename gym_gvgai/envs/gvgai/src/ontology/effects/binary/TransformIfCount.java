...
public class TransformIfCount extends TransformTo {
    // Transform `sprite1` into `stype` if geq <= count(stypeCount) <= leq
    // Otherwise, if `estype` is not null, transform `sprite` to `estype`
    ...
    public void execute(VGDLSprite sprite1, VGDLSprite sprite2, Game game) {
        if (sprite1 == null || sprite2 == null) {
            Logger.getInstance().addMessage(new Message(Message.WARNING,
                    "Neither the 1st nor 2nd sprite can be EOS with TransformIfCount interaction."));
            return;
        }
