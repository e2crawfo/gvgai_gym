    ...
    public void execute(VGDLSprite sprite1, VGDLSprite sprite2, Game game) {
        if (sprite1 == null || sprite2 == null) {
            Logger.getInstance().addMessage(new Message(Message.WARNING,
                    "Neither the 1st nor 2nd sprite can be EOS with WallStop interaction."));
            return;
        }

        // Stop just in front of the wall, removing that velocity component, but
        // possibly sliding along it.

        // Keep in the list, for the current cycle, the sprites that have triggered this
        // event.
        ...
        if (centerXDiff > centerYDiff) {
            // sprite1.orientation = new Direction(0, sprite1.orientation.y() * (1.0 -
            // friction));
            v = new Vector2d(0, sprite1.orientation.y());
        } else {
            // sprite1.orientation = new Direction(sprite1.orientation.x() * (1.0 -
            // friction), 0);
