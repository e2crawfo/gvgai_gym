    ...
    public void execute(VGDLSprite sprite1, VGDLSprite sprite2, Game game) {
        if (sprite1 == null || sprite2 == null) {
            Logger.getInstance().addMessage(new Message(Message.WARNING,
                    "Neither 1st not 2nd sprite can be EOS with BounceDirection interaction."));
            return;
        }

        sprite1.rect.intersection(sprite2.rect);
...
//        boolean horizontalBounce = interRect.width < interRect.height;
        ...
        if (relHit < 0) {
            relHit = Math.max(relHit, -1);
        } else {
            relHit = Math.min(relHit, 1); // Keeping it in [-1,1]
        }
