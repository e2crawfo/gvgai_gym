...
public class SpaceshipAvatar extends ShootAvatar {
    ...
    public void shoot(Game game, int idx) {
        ...
        VGDLSprite newOne = game.addSprite(itype[idx],
                new Vector2d(this.rect.x + Math.cos(this.rotation) * this.lastrect.width,
    ...
    public void move() {
        ...
        if (Utils.processMovementActionKeys(getKeyHandler().getMask(), getPlayerID()) == Types.DUP) {
            ...
            facing = new Direction(Math.cos(this.rotation + Math.toRadians(180)),
                    Math.sin(this.rotation + Math.toRadians(180.0)));
            this.physics.activeMovement(this, facing, speed);
        }
    }
