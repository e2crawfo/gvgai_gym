...
public class Chaser extends RandomNPC {
	...
	public void update(Game game) {
		...
		// Update the list of actions that moves me towards/away from each target
		for (VGDLSprite target : targets) {
			movesToward(target);
		}

		// Randomly choose an action that satisfies current goals
		Direction act;
		if (actions.size() == 0) {
	...
	/**
	 * Sets a list with the closest targets (sprites with the type 'stype'), by
	 * distance
	 * 
	...
	protected void closestTargets(Game game) {
		...
		if (spriteIt == null)
			spriteIt = game.getSubSpritesGroup(itype); // Try subtypes
