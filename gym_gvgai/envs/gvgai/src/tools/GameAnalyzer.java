...
public class GameAnalyzer {
	 ...
	 */
	public final ArrayList<String> spawnerTypes = new ArrayList<String>(Arrays
			.asList(new String[] { "SpawnPoint", "Bomber", "RandomBomber", "Spreader", "ShootAvatar", "FlakAvatar" }));
	/**
	 * List of all different interactions cause spawning
	 */
	public final ArrayList<String> spawnInteractions = new ArrayList<String>(
			Arrays.asList(new String[] { "TransformTo", "SpawnIfHasMore", "SpawnIfHasLess" }));
	/**
	 * List of all different interactions cause an object to be solid
	 */
	public final ArrayList<String> solidInteractions = new ArrayList<String>(
			Arrays.asList(new String[] { "StepBack", "UndoAll" }));
	/**
	 * List of all different interactions cause an object to die
	 */
	public final ArrayList<String> deathInteractions = new ArrayList<String>(Arrays.asList(
			new String[] { "KillSprite", "KillIfHasMore", "KillIfHasLess", "KillIfFromAbove", "KillIfOtherHasMore" }));
	/**
	 * List of all horizontal moving avatar
	 */
	public final ArrayList<String> horzAvatar = new ArrayList<String>(
			Arrays.asList(new String[] { "FlakAvatar", "HorizontalAvatar" }));
	/**
	 * Resource string
	 */
	...
	private void findSolidObjects(GameDescription game) {
		...
		for (SpriteData sprite : avatars) {
			for (SpriteData solid : staticSprites) {
				...
				ArrayList<InteractionData> secondaryInteraction = getAllInteractions(solid.name, InteractionType.FIRST,
						game);
	 ...
	 * @param type  type of checked interaction (if the sprite is always on left or
	 *              right or don't care)
	 * @param game  game description object for the current game
		...
		for (SpriteData s : allSprites) {
			if (spawnerTypes.contains(s.type)) {
				for (String stype : s.sprites) {
					if (harmfulSprites.contains(stype) && !harmfulSprites.contains(s.name)) {
						harmfulSprites.add(s.name);
					}
				}
			}
	...
	/**
	 * get an array list of all other sprites that are not listed in the previous
	 * lists
	 * 
