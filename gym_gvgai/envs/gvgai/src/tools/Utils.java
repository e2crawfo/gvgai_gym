...
/**
 * Created with IntelliJ IDEA. User: Diego Date: 17/10/13 Time: 12:13 This is a
...
public class Utils {
    ...
    public static Polygon trianglePoints(Rectangle rect, Direction orientation) {
        Vector2d p1 = new Vector2d(rect.getCenterX() + orientation.x() * rect.getWidth() / 2.0,
        ...
        Vector2d p2a = new Vector2d(p2.x - orthdir.x * rect.getWidth() / 3.0,
                p2.y - orthdir.y * rect.getHeight() / 3.0);
        Vector2d p2b = new Vector2d(p2.x + orthdir.x * rect.getWidth() / 3.0,
                p2.y + orthdir.y * rect.getHeight() / 3.0);
    ...
    public static double noise(double input, double epsilon, double random) {
        if (input != -epsilon) {
            ...
            // System.out.format("utils.tiebreaker(): WARNING: value equal to epsilon:
            // %f\n",input);
            return (input + epsilon) * (1.0 + epsilon * (random - 0.5));
        }
    }
    ...
    /**
     * Processes a file with game names (containing full path). Assumes that the
     * input is csv, with each line with the format: N, fullPath Where N is the
     * index and fullPath the full path to the file, including extension (.txt).
     *
