...
public abstract class Comm extends Thread {
     ...
     */
    protected Types.LEARNING_SSO_TYPE lastSsoType = Types.LEARNING_SSO_TYPE.JSON; // Type of message chosen by player
                                                                                  // (JSON/Image)
    ...
    /***
     * This method is used to set the game state to either "ABORT_STATE" or
     * "END_STATE" depending on the termination of the game. Each game calls this
     * method upon teardown of the player object.
     *
     * END_STATE: Game ends normally ABORT_STATE: Game is violently ended by player
     * using "ABORT" message or ACTION_ESCAPE key
     *
     * @param so State observation of the game in progress to be used for message
     *           sending.
    ...
    public int finishGame(StateObservation so) {
        ...
        try {
            // Set the game state to the appropriate state and the millisecond counter, then
            // send the serialized observation.
...
//                System.err.println("PipeComm:END_OVERSPENT");
            ...
            // response.equals("END_VALIDATION")) {
    ...
    /**
     * This function is called at the beginning of the game for initialization. Will
     * give up if no "START_DONE" received after having received 11 responses
     */
    public boolean startComm() {

        try {

            // First thing we recieve: ACK from client about connection. We don't care about
            // that, skip.
    ...
    /**
     * This function is called at the end of the whole process. Closes the
     * communication. Will give up if no "START_DONE" received after having received
     * 11 responses
     */
