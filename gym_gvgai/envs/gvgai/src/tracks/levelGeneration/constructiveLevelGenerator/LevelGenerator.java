...
public class LevelGenerator extends AbstractLevelGenerator {
    ...
    /**
     * tradeoff between intended percentage of objects and the generated one from
    ...
    /**
     * Calculate the percentage covered from the level and percentages of each data
     * type
     * 
    ...
    private LevelCoverData getPercentagesCovered(GameDescription game) {
        ...
        for (SpriteData sprite : game.getAllSpriteData()) {
            ...
            // number of collectable sprites (priority value, if not spawned and 0
            // otherwise)
        ...
        data.levelPercentage = (coverTradeOffPercentage
                + (1 - coverTradeOffPercentage) / (numberOfCreationalObjects + 1))
        ...
        if (solidValue > 0) {
            data.solidPercentage = Math
                    .max(Utils.noise(solidValue / totalValue, randomnessPercentage, random.nextDouble()), 0);
        }
        if (harmfulValue > 0) {
            data.harmfulPercentage = Math
                    .max(Utils.noise(harmfulValue / totalValue, randomnessPercentage, random.nextDouble()), 0);
        }
        if (collectableValue > 0) {
            data.collectablePercentage = Math
                    .max(Utils.noise(collectableValue / totalValue, randomnessPercentage, random.nextDouble()), 0);
        }
        if (otherValue > 0) {
            data.otherPercentage = Math
                    .max(Utils.noise(otherValue / totalValue, randomnessPercentage, random.nextDouble()), 0);
        }
    ...
    private void buildLevelLayout(LevelData level, LevelCoverData coverPercentage) {
        ...
        if (solidSprites.size() > 0) {
            ...
            double solidNumber = coverPercentage.levelPercentage * coverPercentage.solidPercentage * getArea(level);
            ...
            while (solidNumber > 0) {
                ...
                ArrayList<Point> directions = new ArrayList<Point>(Arrays
                        .asList(new Point[] { new Point(1, 0), new Point(-1, 0), new Point(0, -1), new Point(0, 1) }));
                while (length > 0) {
                    ...
                    for (i = 0; i < directions.size(); i++) {
                        Point newPoint = new Point(randomPoint.x + directions.get(i).x,
                                randomPoint.y + directions.get(i).y);
    ...
    private Point addAvatar(LevelData level, GameDescription game) {
        ...
        // if the avatar is horizontal mover or shooter it placed at the top or the
        // bottom
        // free positions
     ...
     */
    private void addHarmfulObjects(GameDescription game, LevelData level, LevelCoverData coverPercentage,
            Point avatarPosition) {
        ...
        while (numberOfHarmful > 0) {
            ...
            // if the harmful object is moving then get a far position from the player
            // starting point
    ...
    public String generateLevel(GameDescription game, ElapsedCpuTimer elapsedTimer) {
        ...
        int width = (int) Math.max(minSize + size, game.getAllSpriteData().size()
                * (levelSizeMaxPercentage - levelSizeRandomPercentage + levelSizeRandomPercentage * random.nextDouble())
                + size);
        int length = (int) Math.max(minSize + size, game.getAllSpriteData().size()
                * (levelSizeMaxPercentage - levelSizeRandomPercentage + levelSizeRandomPercentage * random.nextDouble())
                + size);
