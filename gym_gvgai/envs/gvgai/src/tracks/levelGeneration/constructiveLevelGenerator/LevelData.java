...
public class LevelData {
        ...
        for (int y = 0; y < level[0].length; y++) {
            for (String[] element : level) {
                if (element[y] == null) {
                    ...
                    if (!levelMapping.containsKey(element[y])) {
                        levelMapping.put(element[y], mapChar);
                        mapChar += 1;
                    }
                    result += levelMapping.get(element[y]);
                }
            }
            result += "\n";
        }
     ...
     * @return true if the wall didn't split the world into two halves and false
     *         otherwise
     */
    ...
    /**
     * Special point class that is used to get neighbors and handle other useful
     * functions
     * 
     * @author AhmedKhalifa
     */
