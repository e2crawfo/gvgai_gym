...
public class Chromosome implements Comparable<Chromosome> {
    ...
    public void InitializeConstructive() {
        String[] levelString = SharedData.constructiveGen
                .generateLevel(SharedData.gameDescription, null, level[0].length, level.length).split("\n");
    ...
    public void mutate() {
        ...
        for (int i = 0; i < SharedData.MUTATION_AMOUNT; i++) {
            ...
            if (SharedData.random.nextDouble() < SharedData.INSERTION_PROB) {
                ...
                ArrayList<SpritePointData> freePositions = getFreePositions(
                        new ArrayList<String>(Arrays.asList(new String[] { spriteName })));
    ...
    /**
     * get the free positions in the current level (that doesnt contain solid or
    ...
    private void FixPlayer() {
        ...
        if (avatarPositions.size() == 0) {
            ...
            level[freePositions.get(index).y][freePositions.get(index).x]
                    .add(avatarNames.get(SharedData.random.nextInt(avatarNames.size())));
        }
     ...
     * 
     * @return Level mapping object that can help to construct the level string and
    ...
    public LevelMapping getLevelMapping() {
        ...
        char c = 'a';
        for (ArrayList<String>[] element : level) {
            for (int x = 0; x < element.length; x++) {
                if (levelMapping.getCharacter(element[x]) == null) {
                    levelMapping.addCharacterMapping(c, element[x]);
                    c += 1;
                }
            }
        }
     ...
     * @return string of letters defined in the level mapping that represent the
     *         level
     */
    public String getLevelString(LevelMapping levelMapping) {
        ...
        for (ArrayList<String>[] element : level) {
            for (int x = 0; x < element.length; x++) {
                levelString += levelMapping.getCharacter(element[x]);
            }
            levelString += "\n";
        }
    ...
    private HashMap<String, Integer> calculateNumberOfObjects() {
        ...
        for (ArrayList<String>[] element : level) {
            for (int x = 0; x < element.length; x++) {
                ArrayList<String> sprites = element[x];
    ...
    /**
     * Get fitness value for the current score difference between the best player
     * and the naive player
     * 
     * @param scoreDiff difference between the best player score and the naive
     *                  player score
    ...
    /**
     * check if the player death terminates the game player ID used is 0, default
    ...
    /**
     * get a fitness value for the number of unique rules satisfied during playing
     * the game
     * 
    ...
    public ArrayList<Double> calculateFitness(long time) {
        if (!calculated) {
            ...
            if (SharedData.gameAnalyzer.getMinScoreUnit() > 0) {
                double numberOfUnits = SharedData.gameAnalyzer.getMaxScoreUnit()
                        / (SharedData.MAX_SCORE_PERCENTAGE * SharedData.gameAnalyzer.getMinScoreUnit());
            ...
            System.out.println("SolutionLength:" + bestSol.size() + " doNothingSteps:" + doNothingLength
                    + " coverPercentage:" + coverPercentage + " bestPlayer:" + bestState.getGameWinner());
