...
public class LearningMachine {
    ...
    /**
     * Reads and launches a game for an agent to be played. Graphics can be on or
     * off.
     *
     * @param game_file  game description file.
     * @param level_file file with the level to be played.
     * @param visuals    true to show the graphics, false otherwise. Training games
     *                   have never graphics set to ON.
     * @param cmd        array with name of the script file to run for the client,
     *                   plus agent and port
     ...
     */
    public static double[] runOneGame(String game_file, String level_file, boolean visuals, String[] cmd,
            String actionFile, int randomSeed) throws IOException {
...
//
    ...
    /**
     * Reads and launches a game to be played on a series of both pre-determined and
     * non pre-determined levels.
     *
     * @param game_file   game description file.
     * @param level_files file with the level to be played.
     * @param cmd         array with name of the script file to run for the client,
     *                    plus agent and port
     * @param actionFiles filename of the file where the actions of this player, for
     *                    this game, should be recorded.
     */
    public static void runMultipleGames(String game_file, String[] level_files, String cmd[], String[] actionFiles,
            boolean visuals) throws IOException {
        VGDLFactory.GetInstance().init(); // This always first thing to do.
        VGDLRegistry.GetInstance().init();
        CompetitionParameters.IS_LEARNING = true;
        // Create the player.
        ...
        // Playable levels somehow governed by Types.NUM_TRAINABLE_LEVELS (not in this
        // file or LearningPlayer)
    ...
    private static double[] playOnce(LearningPlayer player, String actionFile, String game_file, String level_file,
        ...
        if (player == null
                || LearningMachine.initPlayer(player, actionFile, randomSeed, false, toPlay.getObservation()) == null) {
    ...
    /**
     * Reads and launches a game for a bot to be played. It specifies which levels
     * to play and how many times. Filenames for saving actions can be specified.
     * Graphics always on.
     *
     * @param game_file   game description file.
     * @param level_files array of level file names to play.
     * @param level_times how many times each level has to be played.
     * @param actionFiles names of the files where the actions of this player, for
     *                    this game, should be recorded. Accepts null if no
     *                    recording is desired. If not null, this array must contain
     *                    as much String objects as level_files.length*level_times.
     */
    public static void runGymGames(String game_file, String[] level_files, int level_times, LearningPlayer player,
            String[] actionFiles, boolean visual) throws IOException {
        VGDLFactory.GetInstance().init(); // This always first thing to do.
        VGDLRegistry.GetInstance().init();
        CompetitionParameters.IS_LEARNING = true;
...
//                vict + " , " + sc);
        ...
        // Finally, when the game is over, we need to finish the communication with the
        // client.
    ...
    public static void runGames(String game_file, String[] level_files, int level_times, LearningPlayer player,
            String[] actionFiles, boolean visual) throws IOException {
        VGDLFactory.GetInstance().init(); // This always first thing to do.
        VGDLRegistry.GetInstance().init();
        CompetitionParameters.IS_LEARNING = true;
        ...
        while (keepPlaying && level_idx < trainingLevels.length) {
            ...
            for (int i = 0; keepPlaying && i < level_times; ++i) {
                levelOutcome = playOneLevel(game_file, level_file, i, false, visual, recordActions, level_idx, players,
                        actionFiles, toPlay, scores, victories);
//                System.err.println("levelOutcome="+levelOutcome);
        ...
        if (levelOutcome != Types.LEARNING_FINISH_ROUND) {
            // We only continue playing if the round is not over.
            System.out
                    .println("[PHASE] Starting Second Phase of Training in " + Types.NUM_TRAINING_LEVELS + " levels.");
            while (levelOutcome >= 0) {
                // Play the selected level once
                levelOutcome = playOneLevel(game_file, level_files[levelOutcome], 0, false, visual, recordActions,
                        levelOutcome, players, actionFiles, toPlay, scores, victories);
...
//                System.err.println("validation_level=" + validation_level);
                    levelOutcome = playOneLevel(game_file, validation_level, i, true, visual, recordActions,
                            level_idx + Types.NUM_TRAINING_LEVELS, players, actionFiles, toPlay, scores, victories);
...
//                vict + " , " + sc);
    ...
    /**
     * Method used to play a single given level. It is also used to request player
     * input in regards to the next game to be played.
     *
     * @param game_file     Game file to be used to play the game. Is sent by parent
     *                      method.
     * @param level_file    Level file to be used to play the game. Is sent by
     *                      parent method.
     * @param level_time    Integer denominating how many times the current level
     *                      has been played in a row. Is also sent from the
     *                      exterior, and exists for debugging only.
     * @param isValidation  Indicates if the level being played is a validation
     *                      level
     * @param recordActions Boolean determining whether the actions should be
     *                      recorded.
     * @param levelIdx      Level index. Used for debugging.
     * @param players       Array of Player-type objects. Used to play the game
     * @param actionFiles   Files used to record the actions in for logging
     *                      purposes.
     * @param toPlay        The game to be played. Must be pre-initialized.
     * @param scores        Array of scores to be modified. Is modified at the end
     *                      of the level.
     * @param victories     Array of victories to be modified. Is modified at the
     *                      end of the level.
     * @return Next level to be played as chosen by the player, or a random
     *         substituent.
     * @throws IOException
     */
    public static int playOneLevel(String game_file, String level_file, int level_time, boolean isValidation,
            boolean isVisual, boolean recordActions, int levelIdx, LearningPlayer[] players, String[] actionFiles,
            Game toPlay, StatSummary[] scores, StatSummary[] victories) throws IOException {
        if (VERBOSE) {
        ...
        LearningPlayer learningPlayer = LearningMachine.initPlayer(players[0], actionFiles[0], randomSeed, isValidation,
...
//        System.out.println("LearningMachine required level="+level);
    ...
    /**
     * Creates a player given its name. This method starts the process that runs
     * this client.
     *
     * @param cmd name of the script to execute, with parameters (agent name and
     *            port). If cmd[0] is null, we (the server) is not starting the
     *            communication, the client is, via sockets.
     * @return the player, created but NOT initialized, ready to start playing the
     *         game.
     */
     ...
     * @param actionFile   filename of the file where the actions of this player,
     *                     for this game, should be recorded.
     * @param randomSeed   Seed for the sampleRandom generator of the game to be
     *                     played.
    ...
    private static LearningPlayer initPlayer(LearningPlayer player, String actionFile, int randomSeed,
            boolean isValidation, StateObservation so) {
     ...
     *
     * @param playerName name of the agent to create. It must be of the type
     *                   "<agentPackage>.Agent".
     * @param actionFile filename of the file where the actions of this player, for
     *                   this game, should be recorded.
    ...
    private static LearningPlayer initMultiPlayer(LearningPlayer playerName, String actionFile,
            StateObservationMulti so, int randomSeed, int id, boolean isHuman) {
        return playerName;
    }
    ...
    /**
     * Tears multiple players down. This initiates the saving of actions to file. It
     * should be called when the game played is over. Not useful for singleLearning
     *
