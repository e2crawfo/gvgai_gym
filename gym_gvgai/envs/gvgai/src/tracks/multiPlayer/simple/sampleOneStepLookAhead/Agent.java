...
public class Agent extends AbstractMultiPlayer {
    ...
    /**
     *
     * Very simple one step lookahead agent. Pass player ID to all state observation
     * methods to query the right player. Omitting the player ID will result in it
     * being set to the default 0 (first player, whichever that is).
     *
     * @param stateObs     Observation of the current state.
     * @param elapsedTimer Timer when the action returned is due.
    ...
    // Returns an action, at random, that the oppponet would make, assuming I do
