...
public class MultiSpriteCounter extends Termination {
    // TODO if needed: Theoretically, we could have an array of types here... to be
    // done.
    ...
    public boolean isDone(Game game) {
        ...
        if (itype1 != -1) {
            countAcum += game.getNumSprites(itype1) - game.getNumDisabledSprites(itype1);
        }
        if (itype2 != -1) {
            countAcum += game.getNumSprites(itype2) - game.getNumDisabledSprites(itype2);
        }
        if (itype3 != -1) {
            countAcum += game.getNumSprites(itype3) - game.getNumDisabledSprites(itype3);
        }
