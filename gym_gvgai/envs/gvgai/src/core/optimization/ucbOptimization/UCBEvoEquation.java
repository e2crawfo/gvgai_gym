...
/**
 * implementation of the evolved ucb equations from Evolving game-specific UCB
 * alternatives for General Video Game Playing
 * 
 * @author AhmedKhalifa
 */
public class UCBEvoEquation implements UCBEquation {
    ...
    public double evaluate(double[] values, double[] parameters) {
        ...
        double uctValue = parameters[0]
                * (values[Helper.TREE_CHILD_VALUE] / (values[Helper.TREE_CHILD_VISITS] + UCBEvoEquation.epsilon))
                + parameters[1] * values[Helper.TREE_CHILD_MAX_VALUE]
                + parameters[2]
                        * Math.pow(Math.log(values[Helper.TREE_PARENT_VISITS]) / values[Helper.TREE_CHILD_VISITS]
                                + UCBEvoEquation.epsilon, parameters[3])
                + parameters[4]
                        * Math.pow(values[Helper.SPACE_EXPLORATION_VALUE] + UCBEvoEquation.epsilon, parameters[5])
                + parameters[6] * Math.pow(values[Helper.DISTANCE_MIN_NPC] + UCBEvoEquation.epsilon, parameters[7])
                + parameters[8] * Math.pow(values[Helper.DISTANCE_MIN_PORTAL] + UCBEvoEquation.epsilon, parameters[9])
                + parameters[10]
                        * Math.pow(values[Helper.DISTANCE_MIN_MOVABLE] + UCBEvoEquation.epsilon, parameters[11])
                + parameters[12]
                        * Math.pow(values[Helper.DISTANCE_MIN_RESOURCE] + UCBEvoEquation.epsilon, parameters[13]);
    ...
    public String toString(double[] parameters) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            term[i] = !(Math.abs(temp[i]) <= UCBEvoEquation.epsilon);
        }

        String result = "UCB = " + (term[0] ? temp[0] + " * average(X(j)) + " : "")
