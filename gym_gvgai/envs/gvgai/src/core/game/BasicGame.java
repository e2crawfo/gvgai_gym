...
public class BasicGame extends Game {
	 ...
	 * super.block_size is set to this variable. square_size should be divisible by
	 * all speeds in the game definition.
	 */
	public int square_size;

	/**
	 * List of sprites that should not be traversable for the pathfinder. This list
	 * can be specified with sprite string identifiers separated by commas.
	 */
	public String obs;
			...
			for (int j = 0; j < size.width; ++j) {
				...
				if (charMapping.containsKey(c)) {
					for (String obj : charMapping.get(c)) {
						...
						for (int x = -1; x <= 1; x++) {
							for (int y = -1; y <= 1; y++) {
								if (Math.abs(x) != Math.abs(y) && j + x >= 0 && j + x < size.width && i + y >= 0
										&& i + y < size.height) {
					...
					Logger.getInstance().addMessage(
							new Message(Message.WARNING, "\"" + c + "\" is not defined in the level mapping."));
				}
			}
		}
