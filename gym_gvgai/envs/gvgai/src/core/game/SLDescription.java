...
public class SLDescription {
	...
	/**
	 * constructor for the SLDescription contains information about game sprites and
	 * the current level
	 *
	...
	public SpriteData[] getGameSprites() {
		...
		for (int i = 0; i < result.length; i++) {
			...
			for (int j = 0; j < result[i].sprites.size(); j++) {
				result[i].changeSpriteName(result[i].sprites.get(j),
						this.encodeName(this.gameSprites[i].sprites.get(j), this.shift));
			}
		}
	...
	public String[][] modifyRules(String[] rules, String[] wins, int seed) {
			...
			for (String part : parts) {
				if (part.toLowerCase().contains(KEYWORD + "_")) {
					...
					if (spriteName.length() > 0) {
						modifiedRules.set(modifiedRules.size() - 1,
								modifiedRules.get(modifiedRules.size() - 1) + temp[0] + spriteName + " ");
					} else {
						Logger.getInstance()
								.addMessage(new Message(Message.WARNING, part + " is undefined in the game."));
					}

				} else {
					modifiedRules.set(modifiedRules.size() - 1,
							modifiedRules.get(modifiedRules.size() - 1) + part + " ");
				}
			}
		}
		...
		for (String win : wins) {
			...
			for (String part : parts) {
				if (part.toLowerCase().contains(KEYWORD + "_")) {
					...
					String spriteName = this.decodeIndex(Integer.parseInt(temp[1]), seed);
					if (spriteName.length() > 0) {
						modifiedWins.set(modifiedWins.size() - 1,
								modifiedWins.get(modifiedWins.size() - 1) + temp[0] + spriteName + " ");
					} else {
						Logger.getInstance()
								.addMessage(new Message(Message.WARNING, part + " is undefined in the game."));
					}
		...
		return new String[][] { modifiedRules.toArray(new String[modifiedRules.size()]),
				modifiedWins.toArray(new String[modifiedWins.size()]) };
	}
	 ...
	 * @return state observation of the current game using the new interaction rules
	 *         and termination conditions return null when there is errors
	 */
	 ...
	 */
	public StateObservation testRules(String[] rules, String[] wins,
			HashMap<String, ArrayList<String>> spriteSetStructure) {
